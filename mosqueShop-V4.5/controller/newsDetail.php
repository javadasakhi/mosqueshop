...
if(isset($_GET['k'])){
    if ($_GET['k']=='tnrld'){
        $newDetailContent=connectDB::query('select LongDescriptionNews from news where category=:category',array(':categoey'=>1));
        ...
        $newDetailContent=connectDB::query('select LongDescriptionNews from news where category=:category',array(':categoey'=>2));
        ...
        $newDetailContent=connectDB::query('select LongDescriptionNews from news where category=:category',array(':categoey'=>3));
        ...
        $newDetailContent=connectDB::query('select LongDescriptionNews from news where category=:category',array(':category'=>5));
        $newDetailTitle=connectDB::query('select smallDescriptionNews from news where category=:category',array(':category'=>5));
        $newImage=connectDB::query('select imageNewsAddress from news where category=:category',array(':category'=>5));
    }
    header('Location:../view/newDetail');
}
?>
